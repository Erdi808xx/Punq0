...
class Moderation(commands.Cog):
    ...
    async def log_action(self, guild, action: str, user: discord.Member, moderator: discord.Member, reason: str = "Yok"):
        ...
        embed = discord.Embed(title=f"🛡️ Moderasyon: {action}", color=discord.Color.red(), timestamp=datetime.datetime.now())
    ...
    async def apply_punishment(self, message, reason):
        ...
        # 1. Warn Message
        embed = discord.Embed(title="⚠️ Uyarı", description=f"{guild.name} sunucusunda uyardıldınız.", color=discord.Color.orange())
        ...
        await message.channel.send(f"🛡️ {author.mention} işlem uygulandı: **{action_taken}** (Sebep: {reason})", delete_after=10)
    ...
    async def mod(self, ctx):
        if ctx.invoked_subcommand is None:
            await ctx.send("Lütfen bir alt komut kullanın: `setup`, `badword`, `link`, `spam`, `test`", delete_after=10)
    ...
    async def test_moderation(self, ctx):
        ...
        embed = discord.Embed(
            title="🛡️ Moderasyon Sistemi Test",
            color=0x9d4edd
        )
        
        embed.add_field(
            name="Durum",
            value="✅ Aktif" if mod_config.get("enabled", False) else "❌ Deaktif",
            inline=True
        )
        
        embed.add_field(
            name="Global Küfür Listesi",
            value=f"{len(self.global_bad_words)} kelime yüklü",
            inline=True
        )
        
        embed.add_field(
            name="Özel Küfür Listesi",
            value=f"{len(mod_config.get('bad_words', []))} kelime",
            inline=True
        )
        
        embed.add_field(
            name="Spam Koruması",
            value="✅ Aktif" if mod_config.get("spam_protection", False) else "❌ Deaktif",
            inline=True
        )
        
        embed.add_field(
            name="Link Koruması",
            value="✅ Aktif" if mod_config.get("link_protection", False) else "❌ Deaktif",
            inline=True
        )
        
        embed.add_field(
            name="Log Kanalı",
            value=f"<#{mod_config.get('log_channel')}>" if mod_config.get('log_channel') else "Ayarlanmamış",
            inline=True
        )
        
        # Örnek küfür kelimeleri göster (ilk 5)
        if self.global_bad_words:
            sample_words = ", ".join(self.global_bad_words[:5])
            embed.add_field(
                name="Örnek Yasaklı Kelimeler",
                value=f"||{sample_words}||...",
                inline=False
            )
        
        embed.set_footer(text="⚠️ Yöneticiler moderasyondan muaf tutulur")
    ...
    async def setup(self, ctx, enabled: bool, channel: discord.TextChannel):
        ...
        await ctx.send(f"Moderasyon sistemi **{status}**. Log kanalı: {channel.mention}")
    ...
    async def badword(self, ctx, action: str, word: Optional[str] = None):
        ...
        if action == "list":
            if not current_words:
                await ctx.send("Hiç yasaklı kelime yok.")
            else:
                await ctx.send(f"Yasaklı Kelimeler: ||{', '.join(current_words)}||")
            return

        if not word:
            await ctx.send("Lütfen bir kelime belirtin.")
            return
        ...
        if action == "add":
            if word not in current_words:
                ...
                await ctx.send(f"Eklendi: ||{word}||")
            else:
                await ctx.send("Bu kelime zaten listede.")
        
        elif action == "remove":
            if word in current_words:
                current_words.remove(word)
                await ctx.send(f"Kaldırıldı: ||{word}||")
            else:
                await ctx.send("Bu kelime listede yok.")
    ...
    async def spam_toggle(self, ctx, enabled: bool):
        ...
        await ctx.send(f"Spam Koruması: **{state}**")
    ...
    async def ban(self, ctx, user: discord.User, *, reason: str = "Sebep belirtilmedi"):
        ...
        await ctx.send(f"✅ **{user}** sunucudan yasaklandı. Sebep: {reason}")
    ...
    async def clear(self, ctx, amount: int):
        if amount < 1:
            await ctx.send("En az 1 mesaj silmelisiniz.")
            return
        
        deleted = await ctx.channel.purge(limit=amount)
        embed = discord.Embed(description=f"🗑️ **{len(deleted)}** mesaj silindi.", color=discord.Color.green())
    ...
    async def nuke(self, ctx):
        ...
        embed = discord.Embed(title="Nuke Başarılı ☢️", description="Bu kanal temizlendi.", color=discord.Color.red())
